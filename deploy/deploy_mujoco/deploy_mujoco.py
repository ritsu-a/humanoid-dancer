...
def get_obs(d: mujoco.MjData, motion_res, device: str = "cpu") -> dict:
    ...
    root_rot = torch.tensor(d.qpos[3:7].astype(np.float32), device=device)
    root_rot = torch.cat((root_rot[1:], root_rot[:1]), dim=0)[None, :] # Convert to xyzw format

    root_vel = torch.tensor(d.qvel[:3].astype(np.float32), device=device)
    root_ang_vel = torch.tensor(d.qvel[3:6].astype(np.float32), device=device)
    ...
    dof_pos = torch.tensor(d.qpos[7:].astype(np.float32), device=device)
    dof_vel = torch.tensor(d.qvel[6:].astype(np.float32), device=device)
    ...
    obs["base_ang_vel"] = root_ang_vel.numpy()
    ...
    obs["base_lin_vel"] = root_vel.numpy()
        ...
        with mujoco.viewer.launch_passive(m, d) as viewer_instance:
            while viewer_instance.is_running() and time.time() - sim_start_time < simulation_duration:
                ...
                if sim_step_counter % control_decimation == 0:
                    ...
                    motion_res = _motion_lib.get_motion_state(
                        motion_ids=[0],
                        motion_times=torch.tensor([motion_times], dtype=torch.float32),
                        offset=None
                    )
                    obs_dict = get_obs(d, motion_res, device="cpu") # Gets ABSOLUTE dof_pos

                    motion_index += 1 
                    obs_buf = np.concatenate((  obs_dict['base_ang_vel'] * 0.25 , 
